...
def generate_task_with_mistakes(cls, topic):
    prompt = (
        ...
        "Придумай РЕАЛИСТИЧНУЮ школьную задачу и реши её.\n"
        "Ты УВЕРЕН, что решил правильно, но на самом деле в решении 1-2 ошибки (не бред, а путаница в формулах или единицах).\n"
        "ПРИМЕР ОШИБКИ: использовал сложение вместо умножения, или забыл перевести единицы.\n"
        "НЕ пиши явный бред (например, 'плотность сахара в воде').\n\n"
        "ФОРМАТ:\n"
...
def generate_smart_fallback(cls, topic):
    ...
    # Запасные задачи, если ИИ сгенерировал ерунду
    fallbacks = {
        7: {
...
def check_teacher_quality(message):
    ...
    bad_phrases = ["не знаю", "подумай", "сам", "перечитай", "кажется, нет", "неверно", "неправильно"]
...
def get_student_response(user_message, session):
    ...
    # Инструкции для каждого состояния
    if action == "STAY_CONFUSED":
        instr = (
            "Учитель ответил нечетко или просто сказал 'нет' без объяснений.\n"
            "Твоя реакция:\n"
            "- НЕ соглашайся.\n"
            "- Удивись: 'Но я же всё сделал по формуле!'\n"
            "- Защити своё решение (ссылайся на свои числа).\n"
            "- Спроси: 'А в чем именно ошибка? В цифрах?'"
        )
    ...
    prompt = (
        ...
        f"ИНСТРУКЦИЯ:\n{instr}\n\n"
        "ВАЖНО: Пиши ТОЛЬКО свой ответ. Не пиши 'Учитель:'. Не задавай общих вопросов про физику ('почему формула такая?'), спрашивай только про свою задачу."
    )
