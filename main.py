...
def get_fallback_task(cls, topic):
    ...
    # Шаблоны без меток ошибок (чистый текст)
    fallbacks = {
        7: {
...
def get_student_response(user_message, session):
    ...
    if action == "STAY_CONFUSED":
        instr = (
            "Учитель не объяснил ошибку. Он сказал 'нет' или ушел от ответа.\n"
            "Твоя реакция:\n"
            "1. Ты УВЕРЕН в своём решении.\n"
            "2. Спроси прямо: 'Так ответ правильный или нет?'.\n"
            "3. Запрещено: рассуждать вслух, перечислять варианты, писать мысли в скобках."
        )
    elif action == "ASK_EXAMPLE":
        instr = "Учитель объяснил ошибку. Скажи 'О, понял...' и попроси пример из жизни. Не исправляй решение."
    elif action == "PARTIAL_FIX":
        instr = "Учитель привел пример. Скажи 'А, точно!'. Исправь формулу, но ошибись в счете. Спроси 'Так правильно?'."
    elif action == "ALMOST_THERE":
        instr = "Реши почти правильно, ошибись в единицах. Спроси 'Верно?'."
    else: 
    ...
    prompt = (
        ...
        "СТРОГИЕ ЗАПРЕТЫ:\n"
        "1. НЕ пиши внутренний монолог (например: 'Нет, высота правильная...').\n"
        "2. НЕ пиши мысли в скобках.\n"
        "3. Пиши ТОЛЬКО один короткий вопрос или фразу.\n"
        "4. Не рассуждай о физике."
    )
    ...
    try:
        ...
        # Обрезка, если модель начала "думать"
        if "\n" in result:
             result = result.split("\n")[0]
