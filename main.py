...
def generate_task_with_mistakes(cls, topic):
    # Определяем допустимый уровень сложности
    if cls <= 8:
        level_instruction = (
            "Используй ТОЛЬКО простые формулы (v=s/t, m=ρV, F=mg, A=F*s).\n"
            "НЕ используй синусы, косинусы, сложную динамику или производные.\n"
            "Задача должна быть на равномерное движение или простые силы."
        )
    else:
        level_instruction = (
            "Можно использовать формулы с производными, тригонометрией и законами сохранения."
        )
    ...
    prompt = (
        ...
        "КРИТИЧЕСКИ ВАЖНО:\n"
        "1. Твое решение ДОЛЖНО содержать ошибку (например, перепутаны действия в формуле: умножение вместо деления).\n"
        "2. Задача должна быть короткой и понятной.\n"
...
def generate_smart_fallback(cls, topic):
    ...
    fallbacks = {
        7: {
            ...
            "плотность": f"Найдите плотность вещества, если масса {m} кг, а объём {m*2} м³.\nМОЁ РЕШЕНИЕ:\n1) m - масса, V - объём\n2) ρ = m + V (Ошибка: сложение!)\n3) ρ = {m} + {m*2} = {m*3}\nОТВЕТ: {m*3} кг/м³",
            "сила тяжести": f"Кирпич массой {m} кг лежит на столе. Найдите силу тяжести. (g=10)\nМОЁ РЕШЕНИЕ:\n1) F - сила, m - масса\n2) F = m + 10 (Ошибка: сложение!)\n3) F = {m} + 10 = {m+10}\nОТВЕТ: {m+10} Н"
        },
        8: {
            "работа и мощность": f"Ящик передвинули на {s} м, прилагая силу {F} Н. Какая работа совершена?\nМОЁ РЕШЕНИЕ:\n1) A - работа, F - сила, s - путь\n2) A = F - s (Ошибка: вычитание!)\n3) A = {F} - {s} = {F-s}\nОТВЕТ: {F-s} Дж",
            "простые механизмы": f"С помощью рычага поднимают груз {m*10} кг. Какую силу прикладывают к длинному плечу, если оно в 2 раза больше короткого?\nМОЁ РЕШЕНИЕ:\n1) F1 - сила, F2 - вес груза.\n2) F1 = F2 * 2 (Ошибка: умножение вместо деления!)\n3) F2 = {m*10}*10={m*100} Н. F1 = {m*100} * 2 = {m*200}\nОТВЕТ: {m*200} Н"
        },
        9: {
            "ток": f"Напряжение 12 В, сопротивление 4 Ом. Найдите ток.\nМОЁ РЕШЕНИЕ:\n1) I - ток, U - напряжение, R - сопротивление\n2) I = U + R (Ошибка: сложение!)\n3) I = 12 + 4 = 16\nОТВЕТ: 16 А",
