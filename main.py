...
def get_fallback_task(cls, topic):
    ...
    fallbacks = {
        7: {
            ...
            "плотность": {
                "condition": (
                    f"УСЛОВИЕ: Металлическая деталь имеет массу {m_g} г и объём {m_g // 4} см³. "
        ...
        8: {
            "работа и мощность": {
                "condition": (
                    f"УСЛОВИЕ: Кран поднимает бетонную плиту массой {m_t} т на высоту {h_m} м. "
            ...
            "энергия": {
                "condition": (
                    f"УСЛОВИЕ: Птица массой 2 кг летит на высоте {h_m} м со скоростью {v_ms} м/с. "
        ...
        10: {
            "законы Кеплера": {
                "condition": (
                    f"УСЛОВИЕ: Планета А обращается вокруг Солнца с периодом 1 год на расстоянии 1 а.е. "
                    f"Планета Б находится на расстоянии 4 а.е. от Солнца. "
            ...
            "тяготение": {
                "condition": (
                    f"УСЛОВИЕ: Два тела массами 6 кг и 10 кг находятся на расстоянии 2 м друг от друга. "
                    f"Найдите силу гравитационного притяжения между ними (в Н). "
                ...
                "correct_formula": "F = G * m1 * m2 / r² = 6.67·10⁻¹¹ * 6 * 10 / 4 ≈ 1.0·10⁻⁹ Н"
            },
            "движение по окружности": {
                "condition": (
                    f"УСЛОВИЕ: Трамвай движется по закруглению радиусом {s_m} м со скоростью {v_ms} м/с. "
        ...
        11: {
            ...
            "колебания": {
                "condition": (
                    ...
                    f"Ускорение свободного падения g = 10 м/с².\n"
                    f"МОЁ РЕШЕНИЕ:\n"
                    f"1) T — период, l — длина, g — ускорение свободного падения.\n"
                    f"2) T = 2π * l / g.\n"
                    f"3) T = 2 * 3.14 * 1 / 10 = 0.628 с.\n"
                    f"ОТВЕТ: 0.628 с."
                ),
...
def clean_response(text, user_message):
    ...
    # Убираем «Ответ учителя: ...» / «Учитель: ...» в начале
    text = re.sub(r'^(Ответ учителя:|Учитель:)\s*', '', text, flags=re.IGNORECASE)
    # Убираем начальные кавычки с цитатой
    if re.match(r'^["\']', text):
        text = re.sub(r'^["\'].*?["\']\s*', '', text)
    # Убираем дословное повторение реплики учителя
...
def get_student_response(user_message, session):
    ...
    # -------------------------------------------------------
    # Уровень понимания и СТРОГИЕ инструкции по поведению
    # -------------------------------------------------------
    if good_count == 0:
        # Первая реплика учителя: только общие рассуждения, никаких формул и чисел
        level_instruction = (
            "Учитель только что впервые намекнул тебе на ошибку. "
            "Твой ответ ДОЛЖЕН содержать ТОЛЬКО общие рассуждения и/или бытовой пример — "
            "без каких-либо формул, математических символов и чисел из условия задачи. "
            "Ты можешь сказать, какие физические величины вообще могут быть нужны, "
            "но не называй формулу и не подставляй значения. "
            "Пример допустимого ответа: «О, то есть нужно знать, "
            "насколько сильно тянешь и на сколько метров поднимаешь? "
            "Как когда поднимаешь тяжёлую сумку на этаж?» "
            "Задай уточняющий вопрос или выскажи общее предположение."
        )
    elif good_count == 1:
        # Вторая реплика: можно назвать нужные величины, но формула всё ещё может быть неточной
        level_instruction = (
            "Учитель объяснил тебе один раз. Ты начинаешь понимать. "
            "Можешь назвать, какие величины нужны для решения, "
            "и попробовать вспомнить формулу, но можешь ошибиться в ней или перепутать операцию. "
            "Не пиши полного решения с подстановкой всех чисел."
        )
    elif good_count == 2:
        # Третья реплика: почти правильное решение, но возможна ошибка в вычислениях или единицах
        level_instruction = (
            "Учитель объяснил дважды. Ты почти разобрался. "
            "Можешь написать формулу и попробовать подставить числа, "
            "но можешь ошибиться в вычислениях или единицах измерения."
        )
    else:
        # Четвёртая и последующие реплики: правильное решение
        level_instruction = (
            "Учитель объяснил несколько раз. Теперь ты понял тему "
            "и можешь написать правильное решение."
        )
    ...
    prompt = (
        ...
        "- Отвечай только по-русски. НИКАКИХ английских слов.\n"
        "- Отвечай естественно, как ученик, который учится.\n"
        "- Не повторяй слова учителя дословно.\n"
        "- Если учитель задал вопрос — постарайся ответить на него, даже если ошибаешься.\n"
...
def run_tests():
    ...
    cleaned = clean_response("Я думаю, что force тут равна mass умножить на ускорение.", "")
