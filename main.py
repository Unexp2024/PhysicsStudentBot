...
def contains_explanation(text):
    """Проверяет, содержит ли текст явное указание на ошибку (формула, закон, число, наводящий вопрос)."""
    lower = text.lower()
    explaining_phrases = [
...
def check_teacher_quality(message, topic):
    ...
    # Если сообщение содержит вопросительный знак и ключевые слова наводящего вопроса, считаем полезным
    if "?" in message and any(word in lower_msg for word in ["почему", "как", "зачем", "что", "какой", "определите", "вычислите", "найдите", "сколько"]):
        return True
...
def clean_response(text, user_message):
    ...
    if user_message and user_message in text:
        if text.strip() == user_message.strip():
            text = "Я не совсем понял. Можете объяснить ещё раз?"
        else:
...
def get_student_response(user_message, session):
    ...
    # Если учитель дал полезное объяснение — увеличиваем счётчик
    if is_helpful:
    ...
    # Выбор инструкции в зависимости от того, было ли полезное объяснение
    if not is_helpful:
        # Учитель не дал полезного объяснения — просим пояснить
        instructions = (
            "Учитель не объяснил, в чём ошибка, или ответил не по существу.\n"
            "Вежливо попроси указать, что именно не так в твоём решении.\n"
            "Пиши коротко, 1-2 предложения.\n"
            "Примеры: 'А в чём ошибка?', 'Какая формула правильная?', 'Объясните, пожалуйста, что не так.'"
        )
    else:
        # Учитель дал полезное объяснение — выбираем этап
        if good_count == 1:
            instructions = (
                "Учитель дал тебе первый наводящий вопрос или объяснение.\n"
                "Твоя задача: подумать вслух, попробовать рассуждать, задать уточняющий вопрос, но НЕ давать готовую формулу и НЕ вычислять ответ.\n"
                "Пиши коротко, 1-2 предложения. Например: 'О, то есть работа против силы тяжести... А что нужно знать? Наверное, саму силу тяжести и высоту, на которую поднимают?'"
            )
        elif good_count == 2:
            instructions = (
                "Учитель дал тебе ещё одну подсказку. Ты начинаешь понимать, но всё ещё можешь ошибаться.\n"
                "Попробуй применить формулу, но намеренно ошибись в вычислениях или единицах измерения.\n"
                "Спроси: 'Так правильно?'\n"
                "Пример: 'А, точно! Тогда A = m * g * h = 10000 * 10 * 20 = 2000000 Дж. Так верно?'"
            )
        elif good_count == 3:
            instructions = (
                "Учитель объяснил тебе уже дважды. Ты почти разобрался, но иногда путаешь единицы измерения.\n"
            ...
            instructions = (
                "Учитель объяснил несколько раз. Теперь ты понял тему и можешь решить задачу правильно.\n"
    ...
    prompt = (
        ...
        "Теперь напиши свой ответ (только одну короткую фразу или вопрос).\n"
        "Не повторяй фразы учителя дословно. Говори по-русски грамотно.\n"
        "Твой ответ:"
    )
