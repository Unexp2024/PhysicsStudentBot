...
# ------------------------------
# Генерация задач (улучшенные формулировки)
# ------------------------------
    ...
    fallbacks = {
        7: {
            "равнодействующая сил":
                f"УСЛОВИЕ: На тело действуют две силы: {F1} Н и {F2} Н, направленные в противоположные стороны. Определите равнодействующую силу.\n"
            ...
            "коэффициент полезного действия":
                f"УСЛОВИЕ: С помощью механизма совершена полезная работа {A_pol} Дж, полная работа {A_poln} Дж. Вычислите КПД механизма (в процентах).\n"
            ...
            "гидростатическое давление":
                f"УСЛОВИЕ: Вода (плотность ρ = {rho} кг/м³) находится на глубине {h} м. Определите гидростатическое давление на этой глубине. Ускорение свободного падения g = 10 Н/кг.\n"
            ...
            "сила тяжести":
                f"УСЛОВИЕ: Масса груза составляет {m_kg} кг. Определите силу тяжести, действующую на груз. Ускорение свободного падения g = 10 Н/кг.\n"
                f"МОЁ РЕШЕНИЕ:\n1) F — сила тяжести.\n2) F = m + g.\n3) F = {m_kg} + 10 = {m_kg + 10} Н.\nОТВЕТ: {m_kg + 10} Н.",

            "давление":
                f"УСЛОВИЕ: Трактор массой {m_t*1000} кг стоит на дороге. Площадь опоры его гусениц равна {m_t*2} м². Вычислите давление, которое трактор оказывает на дорогу. Ускорение свободного падения g = 10 Н/кг.\n"
        ...
        8: {
            "работа и мощность":
                f"УСЛОВИЕ: Кран поднимает бетонную плиту массой {m_t} т на высоту {h_m} м. Какую работу совершает кран? Ускорение свободного падения g = 10 Н/кг.\n"
                f"МОЁ РЕШЕНИЕ:\n1) A — работа, F — сила, s — высота.\n2) F = m = {m_t} (просто масса). s = {h_m}.\n3) A = {m_t} * {h_m} = {m_t * h_m} Дж.\nОТВЕТ: {m_t * h_m} Дж.",

            "простые механизмы":
                f"УСЛОВИЕ: При помощи рычага рабочий поднимает камень массой 300 кг. Плечо силы, действующей на камень, равно 0,5 м, плечо силы, приложенной рабочим, равно 3 м. Какую силу должен приложить рабочий, чтобы поднять камень? Ускорение свободного падения g = 10 Н/кг.\n"
            ...
            "теплопроводность":
                f"УСЛОВИЕ: Сколько энергии требуется, чтобы нагреть {m_g//100} кг воды от 20°С до кипения? Удельная теплоёмкость воды c = 4200 Дж/(кг·°С).\n"
        ...
        9: {
            ...
            "движение":
                f"УСЛОВИЕ: Поезд тормозит с ускорением 0,5 м/с². Начальная скорость 36 км/ч (10 м/с). Какой путь он пройдёт до полной остановки?\n"
                f"МОЁ РЕШЕНИЕ:\n1) S — путь, v — скорость, a — ускорение.\n2) S = v / a.\n3) S = 10 / 0.5 = 20 м.\nОТВЕТ: 20 м.",

            "импульс":
                f"УСЛОВИЕ: Мяч массой 0,5 кг летит со скоростью 10 м/с. Найдите импульс мяча.\n"
                f"МОЁ РЕШЕНИЕ:\n1) p — импульс.\n2) p = m + v.\n3) p = 0.5 + 10 = 10.5 кг·м/с.\nОТВЕТ: 10.5 кг·м/с.",

            "архимедова сила":
                f"УСЛОВИЕ: Тело объёмом 0,2 м³ полностью погружено в воду. Определите модуль выталкивающей силы (силы Архимеда), действующей на тело. Плотность воды ρ = 1000 кг/м³, ускорение свободного падения g = 10 м/с².\n"
                f"МОЁ РЕШЕНИЕ:\n1) Fa — сила Архимеда.\n2) Fa = ρ * V.\n3) Fa = 1000 * 0.2 = 200 Н.\nОТВЕТ: 200 Н.",

            "ток":
                f"УСЛОВИЕ: Лампа включена в сеть напряжением {U_V} В. Сила тока в лампе равна {R_Om//10} А. Найдите сопротивление лампы.\n"
                f"МОЁ РЕШЕНИЕ:\n1) R — сопротивление.\n2) R = U + I.\n3) R = {U_V} + {R_Om//10} = {U_V + R_Om//10} Ом.\nОТВЕТ: {U_V + R_Om//10} Ом."
        },
        10: {
            "движение по окружности":
                f"УСЛОВИЕ: Трамвай движется по закруглению радиусом {s_m} м со скоростью {v_ms} м/с. Определите центростремительное ускорение трамвая.\n"
                f"МОЁ РЕШЕНИЕ:\n1) a — ускорение.\n2) a = v + R.\n3) a = {v_ms} + {s_m} = {v_ms + s_m} м/с².\nОТВЕТ: {v_ms + s_m} м/с².",

            "работа":
                f"УСЛОВИЕ: Груз массой 100 кг поднимают на высоту {h_m} м за 2 секунды. Какую работу совершает сила тяги? Ускорение свободного падения g = 10 Н/кг.\n"
                f"МОЁ РЕШЕНИЕ:\n1) A = F * s.\n2) F = m * g = 1000 Н.\n3) A = 1000 * {h_m} = {1000 * h_m} Дж.\nОТВЕТ: {1000 * h_m} Дж."
        },
        11: {
            "термодинамика":
                f"УСЛОВИЕ: Газ совершил работу 500 Дж, получив от нагревателя 800 Дж теплоты. Найдите изменение внутренней энергии газа.\n"
                f"МОЁ РЕШЕНИЕ:\n1) ΔU — изменение энергии, A — работа, Q — теплота.\n2) ΔU = A + Q.\n3) ΔU = 500 + 800 = 1300 Дж.\nОТВЕТ: 1300 Дж.",

            "магнитное поле":
                f"УСЛОВИЕ: Прямолинейный проводник длиной 0,5 м с током 4 А помещён в однородное магнитное поле с индукцией 0,2 Тл перпендикулярно линиям поля. Определите силу Ампера, действующую на проводник.\n"
                f"МОЁ РЕШЕНИЕ:\n1) F — сила Ампера.\n2) F = B + I + L.\n3) F = 0.2 + 4 + 0.5 = 4.7 Н.\nОТВЕТ: 4.7 Н."
        }
    }
...
def check_teacher_quality_llm(message):
    ...
    # Вопрос ученика изменён на "Можете объяснить, пожалуйста?"
    prompt = (
        f"Ученик попросил объяснить: 'Можете объяснить, пожалуйста?'. Учитель ответил: \"{message}\"\n\n"
