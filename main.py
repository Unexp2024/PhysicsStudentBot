...
def get_fallback_task(cls, topic):
    ...
    fallbacks = {
        ...
        9: {
            ...
            "импульс": 
                f"УСЛОВИЕ: Мяч массой 5 кг летит горизонтально со скоростью 15 м/с. Чему равен импульс мяча?\n"
                f"МОЁ РЕШЕНИЕ:\n1) p — импульс.\n2) p = m + v.\n3) p = 5 + 15 = 20 кг*м/с.\nОТВЕТ: 20 кг*м/с.",
...
def check_teacher_quality(message):
    ...
    # Маркеры нерелевантности
    bad_markers = ["не знаю", "подумай", "сам", "перечитай", "трудно сказать", "не уверен", "спроси", "сомневаюсь", "думаю", "непонятно", "подожди", "минуту", "сейчас", "погоди", "молодец", "умница"]
    
    word_count = len(message.split())
    
    # Если есть маркер И сообщение короткое -> плохой ответ
    ...
    # LLM проверка для длинных сообщений (строгая)
    prompt = (
        f"Ученик решил задачу и спросил 'Я правильно решил?'. Учитель ответил: \"{message}\"\n\n"
        "Оцени качество ответа учителя.\n"
        "КРИТЕРИЙ: Учитель должен явно указать на ошибку (например, 'нельзя складывать массу и скорость') или дать правильную формулу.\n"
        "Если учитель хвалит, философствует, уходит от ответа или пишет общие слова — это false.\n"
...
def get_student_response(user_message, session):
    ...
    if action == "STAY_CONFUSED":
        instr = (
            "Учитель НЕ указал на конкретную ошибку (он хвалит, уходит от ответа или говорит загадками).\n"
            "Твоя реакция:\n"
            "1. НЕ пытайся объяснять физику или пересчитывать (ты плохо её знаешь!).\n"
            "2. Скажи, что ты уверен в своём решении.\n"
            "3. Спроси прямо: 'Так ответ правильный или нет?'.\n"
            "4. Не используй правильные формулы."
        )
    ...
    prompt = (
        ...
        "3. Не рассуждай о физике, если ты 'плохой ученик'."
    )
