...
# ------------------------------
# Проверка качества ответа учителя (LLM с задержкой)
# ------------------------------
...
def check_teacher_quality_llm(message, topic):
    prompt = (
        ...
        "Оцени, насколько этот ответ помогает ученику найти ошибку. Используй шкалу от 0 до 2:\n"
        "0 — ответ не помогает (например, 'не знаю', 'подумай', 'неверно' без объяснений).\n"
        "1 — ответ даёт подсказку или наводящий вопрос, но не объясняет конкретную ошибку.\n"
        "2 — ответ явно указывает на ошибку, объясняет физический смысл или даёт готовую формулу.\n\n"
        "Верни только число: 0, 1 или 2."
    )
    resp = cerebras_client.chat.completions.create(
        messages=[{"role": "user", "content": prompt}],
        model="llama3.1-8b", max_tokens=10, temperature=0.1
    )
...
# ------------------------------
# Генерация ответа ученика (с замедленным обучением)
# ------------------------------
...
def get_student_response(user_message, session):
    ...
    # Получаем оценку полезности ответа учителя (0,1,2)
    helpful_score = check_teacher_quality(user_message, topic)
    
    # Увеличиваем счётчик только если ответ был очень полезным (score=2)
    # И добавляем случайную задержку: иногда даже при score=2 не увеличиваем (имитация медленного понимания)
    if helpful_score >= 2 and random.random() < 0.7:  # 70% шанс засчитать
        session['good_explanations'] = good_count + 1
        good_count = session['good_explanations']
    elif helpful_score == 1 and random.random() < 0.3:  # 30% шанс засчитать подсказку
        session['good_explanations'] = good_count + 1
        good_count = session['good_explanations']
    ...
    history_text = ""
    if history:
        last_few = history[-8:]  # берём последние 8 сообщений для контекста
        for msg in last_few:
            role = "Учитель" if msg['role'] == 'user' else "Школьник"
            history_text += f"{role}: {msg['content']}\n"
    history_text += f"Учитель: {user_message}\n"
    
    # Уровень понимания в зависимости от good_count
    if good_count == 0:
        understanding = "Ты только что получил задачу и сделал грубую ошибку. Ты пока плохо понимаешь тему и нуждаешься в подробных объяснениях."
    elif good_count == 1:
        understanding = "Учитель один раз хорошо объяснил тебе ошибку. Ты начинаешь понимать, но всё ещё можешь ошибаться в формулах или вычислениях. Отвечай с осторожностью."
    elif good_count == 2:
        understanding = "Учитель объяснил тебе уже дважды. Ты почти разобрался, но иногда путаешь единицы измерения или делаешь арифметические ошибки."
    else:
        understanding = "Учитель объяснил несколько раз. Теперь ты понял тему и можешь решить задачу правильно, но всё равно проверяй себя."
    
    # Специальная инструкция для начальных стадий: не давать правильную формулу сразу
    if good_count == 0:
        instruction = "Не спеши давать полную формулу. Лучше переспроси, уточни детали или выскажи предположение, но не говори 'A = m*g*h'."
    else:
        instruction = "Отвечай естественно, но если знаешь правильный ответ, можешь его дать."
    ...
    prompt = (
        ...
        "- Если учитель задал вопрос — постарайся ответить, но на ранних стадиях можешь ошибаться или просить уточнить.\n"
...
def run_tests():
    ...
    try:
        ...
        assert res >= 1, "LLM не распознал наводящий вопрос"
